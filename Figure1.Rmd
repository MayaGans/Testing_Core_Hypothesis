...
Simulate taxon abundances and subject to methods.
```{r}
for(trt in 1:(nrow(sim.df))){
  ...
  for(i in 1:nsims){
    ...
    tmp.err.PropRead[i] <-  sum((cumsum(sort(colSums(replicateSim)/sum(rowSums(replicateSim)), decreasing = T)) <= 0.75))  -  ncore
    
    # Prop Reps
    #tmp.err.PropReps[i] <- sum(colSums(replicateSim)>10*nrow(replicateSim))  - ncore
    tmp.err.PropReps[i] <- sum(colSums(replicateSim>0) >= (0.5*nsites)) - ncore
