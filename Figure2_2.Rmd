...
sim.df_all <- sim.df_all %>%
  ...
  # there's 8 levels?? Shouldn't we only have 4??) ##yes, I relabeled the extra columns.
    mutate(method = factor(method, levels = c("meanerrorHardCut", "meanerrorHardCutFP_n",
                                            "meanerrorPropRead",  "meanerrorPropReadFP_n",
                                          "meanerrorPropReadReps","meanerrorPropRepsFP_n",
                                          "meanerrorPropReps", "meanerrorPropReadRepsFP_n"), labels = c("Hard Cutoff","Hard Cutoff", "Proportion of Sequence Reads", "Proportion of Sequence Reads","Proportion of Sequence Reads and Replicates","Proportion of Sequence Reads and Replicates", "Proportion of Replicates","Proportion of Replicates" )))
...
sim.df_all <- sim.df_all %>%
    mutate(error = factor(error, levels = c("FP", "um", "TP"), labels = c("False Positive", "TP-FP", "True Positive")))
