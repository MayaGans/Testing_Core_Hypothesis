...
for(trt in 1:(nrow(sim.df))){
  ...
  sim.df$meanerrorHardCut_FP[trt] <- mean(tmp.err.HardCutFP) / (ntaxa - ncore)
  sim.df$meanerrorPropRead_FP[trt] <- mean(tmp.err.PropReadFP) / (ntaxa - ncore)
  sim.df$meanerrorPropReps_FP[trt] <- mean(tmp.err.PropRepsFP) / (ntaxa - ncore)
...
sim.df$PropRead_TP_num[trt] <- mean(tmp.err.PropReadTP) 
  ...
  sim.df$meanerrorHardCut_FP_num[trt] <- mean(tmp.err.HardCutFP) 
  sim.df$meanerrorPropRead_FP_num[trt] <- mean(tmp.err.PropReadFP) 
  sim.df$meanerrorPropReps_FP_num[trt] <- mean(tmp.err.PropRepsFP) 
...
sim.df_gg <- sim.df %>%
  ...
  mutate(method = factor(method, levels = c("HardCut_TP",
                                            "PropRead_TP",  
                                            "PropReps_TP",
                                          "PropReadsReps_TP"), labels = c("Hard Cutoff", "Proportion of Reads", "Proportion of Replicates", "Proportion of Reads and Replicates")))
...
sim.df_gg <- sim.df %>%
  ...
  mutate(method = factor(method, levels = c("meanerrorHardCutFP",
                                            "meanerrorPropReadFP",  
                                            "meanerrorPropRepsFP",
                                          "PropReadReps_FP"), labels = c("Hard Cutoff", "Proportion of Reads", "Proportion of Replicates", "Proportion of Reads and Replicates")))
...
```{#r}
sim.df_gg <- sim.df %>%
  mutate(HardCutoff = (sim.df$HardCut_TP_num - sim.df$meanerrorHardCutFP_num),
         PropReads = (sim.df$PropRead_TP_num - sim.df$meanerrorPropReadFP_num),
         PropReps = (sim.df$PropReps_TP_num - sim.df$meanerrorPropRepsFP_num),
         PropReadReps = (sim.df$PropReadsReps_TP_num - sim.df$PropReadReps_FP_num)) %>%
...
sim.df$HardCut_ratio <- sim.df$HardCut_TP_num - sim.df$meanerrorHardCut_FP_num
sim.df$PropRead_ratio <- sim.df$PropRead_TP_num - sim.df$meanerrorPropRead_FP_num
sim.df$PropReps_ratio <- sim.df$PropReps_TP_num - sim.df$meanerrorPropReps_FP_num
sim.df$PropReadReps_ratio <- sim.df$PropReadReps_TP_num - sim.df$PropReadReps_FP_num

#(sim.df$HardCut_TP_num - sim.df$meanerrorHardCutFP_num)
#(sim.df$PropRead_TP_num - sim.df$meanerrorPropReadFP_num)
#(sim.df$PropReps_TP_num - sim.df$meanerrorPropRepsFP_num)
#(sim.df$PropReadsReps_TP_num - sim.df$PropReadReps_FP_num)
...
#sim.df$PropReadReps_FP <-  sim.df$PropReadReps_FP


#sim.df$HardCut_TP <- sim.df$HardCut_TP 
#sim.df$PropRead_TP <- sim.df$PropRead_TP 
#sim.df$PropReps_TP <- sim.df$PropReps_TP 
#sim.df$PropReadsReps_TP <- sim.df$PropReadsReps_TP
```
...
# new column a name, new column b name, what do we want the values of
  gather(method, mean, c(5:8, 9:12, 27:30)) %>%
  separate(method, into = c('method', 'error'), sep = "_")
  #gather(method, mean, c(5:16,21:24)) %>%
  #gather(method, mean2, 21:24) %>%
...
sim.df_all <- sim.df_all %>%
  ...
  # if you run 
  # unique(sim.df_all$method)
  # there's 8 levels?? Shouldn't we only have 4??) ##yes, I relabeled the extra columns.
